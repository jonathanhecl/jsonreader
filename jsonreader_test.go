...
func TestLoadFileJSONLine(t *testing.T) {
	type args struct {
		filename string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := LoadFileJSONLine(tt.args.filename)
			if (err != nil) != tt.wantErr {
				t.Errorf("LoadFileJSONLine() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if !reflect.DeepEqual(got.Headers, tt.want.Headers) {
				t.Errorf("LoadFileJSONLine() got.Headers = %v, want %v", got.Headers, tt.want.Headers)
			}

			for i := range got.Rows {
				if !reflect.DeepEqual(got.Rows[i], tt.want.Rows[i]) {
					t.Errorf("LoadFileJSONLine() got.Rows[%v] = %v, want %v", i, got.Rows[i], tt.want.Rows[i])
				}
			}
		})
	}
